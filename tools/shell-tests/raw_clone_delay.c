// Direct clone test with delay after clone to let child thread run
#include <stdio.h>
#include <string.h>
#include <errno.h>
#include <unistd.h>
...
// Child function: uses raw write syscall (no libc, no TLS)
...
int main(void) {
    ...
    *(void**)((char*)child_stack + 8) = (void*)0xDEAD;
    ...
    // Delay: give reactor time to schedule the child thread
    volatile int x = 0;
    for (int i = 0; i < 50000000; i++) x++;
    
    printf("[main] delay done, exiting\n");
    return 0;
}
