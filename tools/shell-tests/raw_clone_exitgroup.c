// Clone test — child uses exit_group for synchronization
#include <stdio.h>
#include <string.h>
#include <errno.h>
#include <unistd.h>
...
static int child_fn(void *arg) {
    ...
    // exit_group(0) — terminates entire process, waking parent
...
int main(void) {
    ...
    *(void**)((char*)child_stack + 8) = (void*)0;
    ...
    if (ret < 0) {
        printf("[main] clone FAILED: %ld\n", ret);
        return 1;
    }
    printf("[main] clone OK: child_tid=%ld\n", ret);
    
    // Spin forever — child will exit_group and kill us
    for (;;) {
        syscall(SYS_gettid);
    }
}
