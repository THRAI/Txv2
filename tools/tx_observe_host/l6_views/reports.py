"""L6 text report projections for Python observe tooling."""
...
def analyze(
    ...
    stream = TraceEventStream(records, TraceIntegrity.from_records("legacy-records", records))
    return analyze_projection(ProjectionInput(stream, names, derived), top=top)


def analyze_projection(projection: ProjectionInput, top: int) -> str:
    records = projection.stream.records
    names = projection.names
    derived = projection.derived or build_derived_tables(records)
    spans = derived.spans
