...
if [ -n "$git_remote" ]; then
    ...
    git_home="/home/tx-dhcp-git-$$"
    clone_dir="$git_home/clone"
    git_workspace_ready=1
    if ! $BB mkdir -p "$git_home"; then
        git_workspace_ready=0
        echo "TXDHCP:git-workspace:fail:$git_home"
    else
        export HOME="$git_home"
        echo "TXDHCP:git-workspace:ready:$git_home"
    fi
    ca_bundle=""
    ...
    if [ "$git_workspace_ready" -eq 1 ] \
        && $BB timeout -k 5 90 /usr/bin/git -c http.version=HTTP/1.1 \
        ...
        if [ "$git_workspace_ready" -eq 1 ]; then
