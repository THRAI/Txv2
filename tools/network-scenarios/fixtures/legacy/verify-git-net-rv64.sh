...
timeout 220 qemu-system-riscv64 -machine virt -kernel "$K" -m 1G -nographic -smp 1 -bios default \
  ...
  -append "tx.runsh=/musl/tx-run.sh tx.net.mode=dhcp console=ttyS0${EXTRA_CMDLINE:+ $EXTRA_CMDLINE}" \
  > "$SERIAL" 2>&1
say ""
