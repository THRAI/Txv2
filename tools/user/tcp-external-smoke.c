// External outbound TCP smoke (P2 acceptance載体). connect() to the SLIRP
// gateway 10.0.2.2:8000 (where the host runs `python3 -m http.server`), send a
// minimal HTTP GET, read the status line. Prints tx-n68-ext-ok on a 2xx/HTTP
// response, tx-n68-ext-fail otherwise. Exercises S1 (send SYN) + S2 (receive
// SYN-ACK via IRQ) + multi-byte send/recv end-to-end over the real device.
...
static void fail(void) {
    static const char msg[] = "tx-n68-ext-fail\n";
...
void _start(void) {
    ...
    static const char ok[] = "tx-n68-ext-ok\n";
    char buf[256];

    // 10.0.2.2:8000 — QEMU SLIRP gateway -> host loopback (http.server).
    ...
    // Read the response; expect it to begin with "HTTP/".
