...
pub fn run() -> Result<()> {
    ...
    match cmd.as_str() {
        ...
        "shell-smoke" => shell_smoke::shell_smoke(&root, args.collect()),
    ...
    println!(
        "txKernel xtask\n\n\
         Commands:\n\
           ...
           cargo xtask shell-smoke   (interactive busybox-over-QEMU smoke; needs TX_BUSYBOX, riscv64gc-unknown-none-elf, qemu-system-riscv64)\n\
