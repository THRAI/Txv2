审核下面的文档，没问题的话加到docs/文件夹下：# txKernel vDSO + musl-static Bootstrap ABI Design
...
Minimum fields required for musl-static bootstrap:
...
1. Should munmap(vdso/vvar) be allowed in v1?
   Recommendation: allow, rely on libc fallback.

2. Should AT_SYSINFO_EHDR be omitted until vDSO ELF is valid?
   Recommendation: yes. Do not expose a fake pointer.

3. Should coarse CLOCK_MONOTONIC be returned for CLOCK_MONOTONIC?
   Recommendation: no. Return -ENOSYS until high-res path is correct.

4. Should kernel parse PT_TLS for diagnostics?
   Recommendation: optional debug-only parser outside critical exec path.

5. Should VVAR be global forever?
   Recommendation: global in v1; per-CPU only after getcpu/rseq/migration checks exist.
```
