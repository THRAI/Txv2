...
决赛第一阶段包含CAgent与BuildStorm两项测试。TxKernel完成了两种架构下的CAgent适配，10项测试全部通过；平台该项满分为199.1分，TxKernel在RV64与LA64上均取得满分。BuildStorm的Rust工具链检查和最小项目构建均能通过，平台记录了这两项的20分；本地8核环境下还能够从源码完整编译`arceos-helloworld`并生成有效产物。由于完整流程超过平台单次评测的时间限制，平台未能记录最终编译结果，但本地完整测试可以正常结束。

本地两次完整联合测试中，CAgent均为10/10，BuildStorm计时编译分别耗时1252.93秒和1183.45秒。作为对照，我们在相同的RV64、4 GiB内存和8核QEMU配置下运行Linux基线，其BuildStorm计时编译耗时为675.40秒。TxKernel两次耗时约为Linux基线的185.5%和175.2%，平均约为180.4%，即当前编译耗时约为Linux的1.8倍，编译速度约为Linux的55.4%。这里比较的是BuildStorm正式计时的编译阶段，不包含测试启动和未计时的构建工具预编译过程。

截至榜单截图记录，TxKernel在决赛第一阶段以438.2分排名第22位：

![决赛第一阶段排名](./docs/image/决赛第一阶段排名.png)

相关问题分析、修复设计和实验结果见[决赛第一阶段优化文档](./tx决赛线上修复文档.pdf)。
