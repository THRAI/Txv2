...
- `cargo xtask syscall-status --check` — fails when the autogenerated
  section in `docs/progress/SYSCALL_STATUS.md` drifts from
  `crates/tx-shims/src/linux_syscall/{numbers.rs, mod.rs}`. Re-sync with
  `cargo xtask syscall-status --regen`. See `tx-ltp-syscall` skill.
