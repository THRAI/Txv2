...
`docs/ljs/ltp-runtests.md`，运行入口是
...
| net | 0 | socket/network | deferred | 当前脚本过滤了网络前缀，另见 `ltp-network-deferred.md` |
...
当前内核侧已支持 `p0`、`smoke`、`fd-io`、`vfs`、`vfs-tail`、`vm`、`process`、`cred`、`signal`、`time`、`ipc`、`event`、`sched`、`mount`、`heavy` 和 `aio`。`net` 当前为 0 个 case，网络相关另行记录。
