...
| `accept03` | Focused tail log reports `13/23`: all available non-socket fd errno checks pass, but pidfd/fanotify/inotify/perf/bpf/new mount API/memfd providers are `TCONF` | Direct blocker is broader fd/syscall surface, not TCP accept dataplane. Do not add fake descriptor-producing stubs just to satisfy this fd enumerator. |
