...
- Bus terminal behavior is still untyped first-slice behavior over `u64`
  masks/events. Typed declarations, epoll-style long-lived subscriptions,
  SMP-safe subscriber storage, and epoch-protected destruction remain later.
- CoreInit still runs a finite smoke task and shutdown path. The permanent
  interrupt-driven idle loop remains a later kernel/runtime slice.
