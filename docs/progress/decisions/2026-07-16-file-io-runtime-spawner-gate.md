...
| Kernel reactor implementation | `IO_MANAGER_v1.md` says services are reactor futures; `DEVICE.md` makes the kernel responsible for device execution scheduling | `CoreInit` owns `BOOT_REACTOR` and now installs the file-I/O spawner after boot reactor initialization | Accepted: the kernel adapter builds the long-lived page-service future with the current-hart affinity and drains the registry exactly once. |
