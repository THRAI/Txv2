...
**Last refresh:** 2026-05-29 (refreshed after the selective namespace merge into
the 66-missing worktree; the autogenerated table is current against Linux RV64
v6.17, with local futex2 constants and `statx` listed as intentional
non-reference entries).

## Headline counts

- `pub const NR_*` defined in `numbers.rs`: **269**
- Dispatched in `mod.rs` (per match arms): **262**
- Defined but not dispatched: **7** (`CAPGET`, `CAPSET`, `EPOLL_PWAIT2`,
  `FANOTIFY_MARK`, `INOTIFY_ADD_WATCH`, `INOTIFY_RM_WATCH`, `SETRLIMIT`).
- True missing from local `numbers.rs` vs Linux RV64 v6.17: **55**.
- Number mismatches vs Linux RV64 v6.17: **0**.
- Local `NR_*` extras not in the Linux RV64 v6.17 reference: **4**
  (`FUTEX2_WAKE`, `FUTEX2_WAIT`, `FUTEX2_REQUEUE`, `STATX`).
...
|---|---|---|---|
| Timer/time tail (`timer_create` family, `clock_adjtime`, `adjtimex`) | +15 timer tests mostly landed; residual CPU-time/discipline cases remain | M (3–4w) mostly landed; residuals are new-policy work | wallclock set/get policy, vDSO conversion state, `timerfd`, `nanosleep`, interval timers, POSIX timer ids, `adjtimex`, and `clock_adjtime` are wired with v1 host coverage; CPU accounting counters now feed CPU clocks and `getrusage`, `ITIMER_VIRTUAL`/`ITIMER_PROF` state is accepted, `ADJ_TICK`/`ADJ_TIMECONST` and `ADJ_OFFSET`/`ADJ_FREQUENCY` round-trip as bounded bookkeeping, and POSIX timer `sigev_value` reaches signalfd `ssi_int`/`ssi_ptr`. Remaining gaps are automatic scheduler/trap CPU charging, CPU-timer signal delivery at safe boundaries, true offset/frequency slew discipline, full realtime per-occurrence sigqueueing, and full Linux blocking-syscall restart/remnant semantics |
...
| `io_uring`/AIO tail (`io_uring_register`, user-mmapped ring depth) | +10 async I/O tests | M (3–5w) | raw Linux AIO handles, shared PageBacked Linux-shaped ring header/drain, reactor-worker completion, queued `io_cancel`, `io_pgetevents`, and a nonblocking `io_uring_enter` scaffold exist; io_uring registration/ring-depth and deeper running-request cancellation remain |
...
- **Dispatched (has a match arm):** 273
- **Defined but not dispatched:** 5 — see list below

- **True missing vs Linux RV64 reference:** 46
...
- `pub const NR_*` in numbers.rs: **278**
- Linux RV64 reference syscalls: **320**
- dispatched in mod.rs: **273** (of which async: 77, likely-stub: 3)
- defined but not dispatched: **5**
...
- `NR_SETNS` (268) → `(inline)`
- `NR_UNSHARE` (97) → `(inline)`

### Defined in `numbers.rs` but no dispatch arm (5)

These have a syscall number constant but no match arm in `mod.rs`. Either wire them up or remove the constant.

- `NR_CAPGET` (nr=90)
- `NR_CAPSET` (nr=91)
- `NR_FANOTIFY_MARK` (nr=263)
- `NR_INOTIFY_ADD_WATCH` (nr=27)
- `NR_INOTIFY_RM_WATCH` (nr=28)

### True missing from local `numbers.rs` (46)
...
### Dispatched syscalls (273) — name → handler
...
| 3 | `NR_IO_CANCEL` | `sys_io_cancel` | sync |
...
| 162 | `NR_SETDOMAINNAME` | `sys_setdomainname` | sync |
...
| 179 | `NR_SYSINFO` | `sys_sysinfo` | sync |
...
| 258 | `NR_RISCV_HWPROBE` | `sys_riscv_hwprobe` | sync |
| 259 | `NR_RISCV_FLUSH_ICACHE` | `sys_riscv_flush_icache` | sync |
