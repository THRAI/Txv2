...
**Last refresh:** 2026-05-27 (refreshed after the futex Linux-compat merge and
the AIO/time-accounting WIP restore; the autogenerated table is current against
Linux RV64 v6.17, with local futex2 constants listed as intentional
non-reference entries).

## Headline counts

- `pub const NR_*` defined in `numbers.rs`: **249**
- Dispatched in `mod.rs` (per match arms): **245**
- Defined but not dispatched: **4** (`GETPEERNAME`, `GETSOCKOPT`,
  `SHUTDOWN`, `SOCKETPAIR`).
- True missing from local `numbers.rs` vs Linux RV64 v6.17: **74**.
- Number mismatches vs Linux RV64 v6.17: **0**.
- Local `NR_*` extras not in the Linux RV64 v6.17 reference: **3**
  (`FUTEX2_WAKE`, `FUTEX2_WAIT`, `FUTEX2_REQUEUE`).
...
- **Dispatched (has a match arm):** 253
- **Defined but not dispatched:** 7 — see list below

- **True missing vs Linux RV64 reference:** 66
...
- `pub const NR_*` in numbers.rs: **250**
- Linux RV64 reference syscalls: **320**
- dispatched in mod.rs: **239** (of which async: 70, likely-stub: 3)
- defined but not dispatched: **11**

- true missing vs Linux RV64 reference: **74**
- number mismatches vs Linux RV64 reference: **0**
- local `NR_*` not in Linux RV64 reference: **4**
...
### Defined in `numbers.rs` but no dispatch arm (11)
...
| 120 | `sched_getscheduler` | `pid_t pid` | `kernel/sched/syscalls.c`:1012 |
| 121 | `sched_getparam` | `pid_t pid, struct sched_param *param` | `kernel/sched/syscalls.c`:1042 |
| 124 | `sched_yield` | `` | `kernel/sched/syscalls.c`:1371 |
| 125 | `sched_get_priority_max` | `int policy` | `kernel/sched/syscalls.c`:1479 |
| 126 | `sched_get_priority_min` | `int policy` | `kernel/sched/syscalls.c`:1507 |
| 127 | `sched_rr_get_interval` | `pid_t pid, struct __kernel_timespec *interval` | `kernel/sched/syscalls.c`:1565 |
| 142 | `reboot` | `int magic1, int magic2, unsigned int cmd, void *arg` | `kernel/reboot.c`:728 |
