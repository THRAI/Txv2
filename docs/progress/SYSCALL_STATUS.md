...
The block below this paragraph is **maintained by `cargo xtask syscall-status`**,
which reads `crates/tx-shims/src/linux_syscall/{numbers.rs, mod.rs}` as the
single source of truth. Do not edit between the AUTOGEN markers by hand —
run `cargo xtask syscall-status --regen` after changing the dispatch table,
or `--check` to verify in CI. The human-curated sections further down
(priority table, OSComp/LTP coverage, topic categorization) are intentionally
*not* autogenerated and remain editable.
...
- **`NR_*` defined:** 119
- **Dispatched (has a match arm):** 107
- **Defined but not dispatched:** 12 — see list below
...
| `NR_*` | # | Summary |
|---|---:|---|
| `NR_EPOLL_CREATE1` | 291 | `epoll_create1(flags)`. Linux generic uapi `__NR_epoll_create1 = 291`. |
| `NR_EPOLL_CTL` | 233 | `epoll_ctl(epfd, op, fd, event_ptr)`. Linux generic uapi |
| `NR_EPOLL_PWAIT` | 281 | `epoll_pwait(epfd, events, maxevents, timeout, sigmask)`. |
| `NR_EPOLL_WAIT` | 232 | `epoll_wait(epfd, events, maxevents, timeout)`. Linux generic uapi |
...
| `NR_RT_SIGPENDING` | 136 | `rt_sigpending(set, sigsetsize)` — Linux RV64 `__NR_rt_sigpending = 136`. |
| `NR_RT_SIGQUEUEINFO` | 138 | `rt_sigqueueinfo(pid, sig, info)` — Linux RV64 `__NR_rt_sigqueueinfo = 138`. |
| `NR_RT_SIGSUSPEND` | 133 | `rt_sigsuspend(mask, sigsetsize)` — Linux RV64 `__NR_rt_sigsuspend = 133`. |
| `NR_RT_SIGTIMEDWAIT` | 137 | `rt_sigtimedwait(set, info, timeout, sigsetsize)` — Linux RV64. |
| `NR_SIGALTSTACK` | 132 | `sigaltstack(ss, old_ss)` — Linux RV64 `__NR_sigaltstack`. |
| `NR_SIGNALFD` | 282 | Historical `signalfd(fd, &mask, sizemask)` (no `flags`). x86_64 |

<!-- END AUTOGEN: syscall-table -->

For a quick on-the-fly count without regenerating:

```sh
cargo xtask syscall-status                    # headline + detail-command menu
cargo xtask syscall-status <NAME>             # info for one syscall
cargo xtask syscall-status --list-missing     # defined but not dispatched
```
