...
| `CI-GATE-SYSCALL-STATUS` | `cargo xtask syscall-status --check` | "autogen section is stale" | Run `cargo xtask syscall-status --regen` and commit the regenerated section of `docs/progress/SYSCALL_STATUS.md`. See `tx-ltp-syscall`. |
