...
- **Specific LTP test failure** ("`fcntl04` is hitting `-ENOSYS`"). Trace
  the test back to the syscall it exercises; if you can't tell which `NR_*`
  is at fault, run the failing test under `cargo xtask shell-test` /
  `cargo xtask oscomp run` and grep the trace for `nr=`. The
  `tx-shell-syscall-fixup` skill is the canonical loop for this.
- **A specific syscall by name** ("implement `sendfile`"). Check
  `crates/tx-shims/src/linux_syscall/numbers.rs` for `NR_*`, then
  `mod.rs` for the dispatch arm. State varies:
  - no `NR_*` and no arm → unwired (look it up in `SYSCALL_STATUS.md`'s
    topic sections);
  - `NR_*` defined, arm returns `-ENOSYS` → already-partial (cheaper);
  - arm wired but body buggy → use `tx-shell-syscall-fixup`.
- **"Where should I focus?"** Open `SYSCALL_STATUS.md` and read the
  high-stakes table. Default recommendation: best LTP impact ÷ effort
  (today: `fork` + CLOEXEC, then `getrlimit`/`setrlimit`, then
...
1. Run the recount commands in `SYSCALL_STATUS.md` and update the headline
   counts if they changed. (Eyeball counts drift after every PR; the file
   has the commands.)
2. If your change closed a stubbed arm, remove the entry from
   **Already-partial**.
3. If your change wired a previously-unwired syscall, remove it from its
   topic section and decrement the section count in the heading.
4. If your change shifted the high-stakes table — a row landed, an effort
   ...
   delete it.
5. Update **Last refresh** at the top.
6. Add a one-line entry to `docs/progress/STATUS.md` pointing at the
