...
use crate::vfs::checks::resolution::state::{ResumeToken, TrailEntry, WalkMode, WalkState};
use crate::vfs::checks::resolution::step::{self, IORequest, KernelStep};
    ...
    loop {
        if mode.is_parent_mode() && state.remaining.count() == 1 {
            return match terminal::build_penultimate_witness(mode, state, guard) {
                Ok(witness) => DriverStep::Accept(witness),
...
pub(crate) fn apply_io_result<'t, 'g>(
    ...
    match token {
        ResumeToken::LookupChild {
            ...
            let child = match io_result {
                IOResult::ChildFound(child) => child,
                IOResult::ChildMissing => return Err(Errno::NoEntry),
                IOResult::SymlinkTarget | IOResult::Unsupported => {
        ...
        ResumeToken::ReadSymlink { suspended, .. } | ResumeToken::ProbeFinal { suspended, .. } => {
            let _resumed = suspended.resume(guard);
