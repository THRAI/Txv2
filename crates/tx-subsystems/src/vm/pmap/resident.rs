...
//! future implementations. Production uses the chunked backend so mmap,
//! munmap, exec and process teardown never shift one address-space-wide
//! contiguous array while holding the pmap lock. Tests retain the sorted
//! `Vec` backend for behavior comparison.
