...
/// VM recipe roots use `Published<T>` and therefore share the substrate EBR
/// deferred-drop queue. Keep the former kernel maintenance hook as a no-op so
/// callers do not run a second, competing reclamation path.
pub fn drain_deferred_recipe_reclaims(_limit: usize) -> usize {
    0
}
