...
impl AddressSpace {
    ...
    pub(crate) fn publish_fault_materialization_ref(
        ...
        self.publish_fault_materialization_locked_ref(outcome, materialization)
    }

    /// Publish the final fault result while the caller continuously owns the
    /// target page's Materializer reservation.
    pub(in crate::vm) fn publish_fault_materialization_locked_ref(
        &self,
        outcome: &VmFaultOutcome,
        materialization: VmFaultMaterialization,
    ) -> Result<PmapPublishOutcome, VmFaultError> {
    ...
    /// surface that result; syscall paths use [`Self::fork_aspace_wait`].
    ...
    /// Wait-capable fork preparation for concurrent syscall paths. No range
    /// reservation survives an await; each retry observes the parent anew.
    ...
    fn try_fault_script_materialize_and_publish(
        ...
        // Own the target page continuously from the final observation through
        // private/page-cache materialization, publication revalidation and PTE
        // installation. If the operation must wait, the guard is dropped with
        // this stack frame and the complete attempt is retried.
        ...
        match self.try_fault_script_publish_locked(outcome, materialization) {
            Ok(published) => Ok(published),
    ...
    pub fn try_mmap(&self, request: VmMapRequest) -> Result<VmMapOutcome, VmMapError> {
        self.try_mmap_with_reserve_observer(request, |_, _| {})
    }

    fn try_mmap_with_reserve_observer(
        &self,
        request: VmMapRequest,
        mut before_reserve: impl FnMut(&AddressSpace, UserRange),
        ...
        loop {
            ...
            // The recipe lookup above is an observation, not a reservation.
            // A sibling sharing this AddressSpace may publish into the same
            // gap before reserve_map acquires the RangeLock. Keep this seam
            // explicit so the collision is covered deterministically in the
            // host regression test below.
            before_reserve(self, range);
            ...
            match self.reserve_map(entry, placement) {
                ...
                // mmap(NULL, ...) must choose another free gap when a
                // concurrent thread wins the observed one. EEXIST is only a
                // user-visible result for MAP_FIXED_NOREPLACE.
    ...
    #[cfg(test)]
    pub(crate) fn try_mmap_with_reserve_observer_for_test(
        &self,
        request: VmMapRequest,
        before_reserve: impl FnMut(&AddressSpace, UserRange),
    ) -> Result<VmMapOutcome, VmMapError> {
        self.try_mmap_with_reserve_observer(request, before_reserve)
    }
    ...
    pub async fn mmap_script(&self, request: VmMapRequest) -> Result<VmMapOutcome, VmMapError> {
        loop {
            let (range, placement, anywhere) = match request.target {
                ...
                VmMapTarget::Fixed { range, placement } => (range, placement, false),
            };
            let entry = VmEntry::new(range, request.prot, request.flags, request.backing.clone());
    ...
    pub fn reserve_map(&self, entry: VmEntry, placement: MapPlacement) -> MapReserveResult<'_> {
        let guard = match self
            .range_lock
            .acquire_step_rich(entry.range, LockMode::ExclusiveWriter)
        {
            crate::vm::AcquireResult::Acquired(guard) => guard,
            crate::vm::AcquireResult::WouldBlock(blocked) => {
                let token = blocked.into_wait_token();
    ...
    pub fn madvise(&self, range: UserRange, advice: MadviseAdvice) -> Result<(), VmMapError> {
        match advice {
            MadviseAdvice::DontNeed | MadviseAdvice::Free => {
                let _guard = self.acquire_writer(range)?;
                self.pmap.teardown_range(range)?;
                // Private anonymous/COW frames have both PTE MapPins and
                // recipe-owned CachePins. MADV_DONTNEED must withdraw the
                // latter after the shootdown, otherwise the next fault sees
                // the old bytes and the pages remain charged indefinitely.
