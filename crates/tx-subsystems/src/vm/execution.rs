    ...
    pub fn fork_aspace<P: PmapIf>(parent: &AddressSpace) -> Result<AddressSpace, VmMapError> {
        ...
        for entry in parent_recipes {
            ...
            if private {
                ...
                // 2c97491b — git clone/push/pull Task2; restored after the
                // main merge, whose vm rewrite dropped it.)
                for (page, snap) in parent.pmap.walk_range(range) {
                    let map_pin = step_engine::page_allocator::acquire_map_pin(snap.ppn)
                        .map_err(VmMapError::PageAlloc)?;
                    child.pmap.publish_page(
                        page,
                        snap.ppn,
