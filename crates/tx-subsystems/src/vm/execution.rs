...
impl AddressSpace {
    ...
    pub async fn fault_script(&self, fault: VmFault) -> Result<PmapPublishOutcome, VmFaultError> {
        ...
        // resolution use [`Self::fault_script_for_process_with_post`] (PR-10
...
pub struct UfdDispatchTarget<'a> {
    ...
    /// Owner-aware publication route for the userfaultfd readable edge.
    ///
    /// This is supplied by the caller's reactor context; keeping it explicit
    /// avoids turning a cross-hart wake into an unowned direct mailbox post.
...
async fn dispatch_ufd_fault<D: UfdDispatch>(
    ...
    if let Some(pusher) = target.fault_pusher {
        pusher.push_fault_msg_with_post(
            crate::userfaultfd::UffdMsg {
