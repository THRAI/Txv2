...
impl ProcessIdentity {
    ...
    /// Snapshot Linux `mlockall(MCL_FUTURE)` policy for this process.
    /// Returns `false` for zombies.
    pub fn mlock_future(&self) -> bool {
        self.payload
            .lock()
            .as_ref()
            .is_some_and(|p| p.mlock_future())
    }

    /// Enable or clear Linux `mlockall(MCL_FUTURE)` policy for this
    /// process. No-op for zombies.
    pub fn set_mlock_future(&self, value: bool) {
        if let Some(payload) = self.payload.lock().as_ref() {
            payload.set_mlock_future(value);
        }
    }
...
pub struct ProcessPayload {
    ...
    /// Linux `mlockall(MCL_FUTURE)` process policy.
    ///
    /// When set, later `mmap(2)` calls install VM recipes with
    /// `VmEntryFlags.locked`. The flag is process-local: new process
...
impl ProcessPayload {
    ...
    /// Read Linux `mlockall(MCL_FUTURE)` policy for this process.
    pub fn mlock_future(&self) -> bool {
        self.mlock_future.load(Ordering::Acquire)
    }
