...
//!
//! This is the device execution seam, not a replacement for the legacy
//! `devices.rs` boot path yet.  It builds the final resource graph, prepares a
//! private set of prospective bindings, validates the complete set, and only
//! then freezes a report.  The production entry point retains rollback-safe
//! registry reservations until the complete runtime is ready, publishes once,
//! and only then arms devices and unmasks their typed interrupt routes.
...
impl<P: TxPlatform> DeviceBindReservation<P> {
    fn new(device: &'static PlatformDevice, key: BoundDeviceKey, driver: DriverId) -> Self {
        Self {
            device,
            key,
            driver,
    ...
    B: StaticDeviceBundle<P>,
{
    ...
    for device in devices {
        ...
        let mut reservation = DeviceBindReservation::<P>::new(device, key, driver.id);
        match (driver.prepare)(device, &mut reservation) {
            Ok(()) => match reservation.into_pending() {
                Ok(binding) => pending.push(binding),
