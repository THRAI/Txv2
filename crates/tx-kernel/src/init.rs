...
impl<P: TxPlatform> CoreInit<P> {
    ...
    /// Overlay the mounted image's standard directories for the explicit
    /// `tx.runsh` lane. The normal OSComp root layout is left unchanged.
        ...
        for name in [
            ...
            let ext4_sub_rnode = {
                let raw = RNode::new(ext4_sub_id, ext4_sub_meta, RNodeBacking::Directory)
                    .with_containing_mount(&ext4_payload);
                let Ok(reservation) = step_engine::reserve_for::<RNode>() else {
                    continue;
                };
                step_engine::sign_for(reservation, raw)
            };
            ...
            if let Some(namespace) = init_mount_namespace() {
                namespace.register_mount(&mountpoint_dentry, overlay_mount);
            }
        }
    }
