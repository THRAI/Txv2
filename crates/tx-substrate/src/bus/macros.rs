...
/// Implement [`WireOwnerManifest`](crate::bus::WireOwnerManifest) for an owner
/// that embeds bus wires as fields.
///
/// Each wire entry names an owner field and the retire method to call. The
/// generated implementation retires every listed wire under one epoch guard,
/// combines their retire records into a [`WireOwnerRetireFence`], and wires the
/// typed reclaim callback through the owner manifest.
#[macro_export]
macro_rules! bus_wire_owner_manifest {
    (
        unsafe impl WireOwnerManifest for $owner:ty {
            reclaim_owner($owner_ptr:ident) $reclaim:block

            wires {
                $first_field:ident => $first_method:ident($($first_arg:expr),* $(,)?);
                $(
                    $field:ident => $method:ident($($arg:expr),* $(,)?);
                )*
            }
        }
    ) => {
        unsafe impl $crate::bus::WireOwnerManifest for $owner {
            fn retire_embedded_wires(
                &self,
                guard: &$crate::epoch::Guard<'_>,
            ) -> ::core::result::Result<
                $crate::bus::WireOwnerRetireFence,
                $crate::bus::WireOwnerReclaimError,
            > {
                let mut fence = $crate::bus::WireOwnerRetireFence::from_retirement(
                    self.$first_field.$first_method($($first_arg,)* guard),
                )?;
                $(
                    fence.include(self.$field.$method($($arg,)* guard))?;
                )*
                Ok(fence)
            }

            unsafe fn reclaim_owner($owner_ptr: ::core::ptr::NonNull<Self>) {
                $reclaim
            }
        }
    };
