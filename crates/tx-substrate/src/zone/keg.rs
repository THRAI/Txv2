...
impl<T: 'static> Keg<T> {
    ...
    pub(crate) fn snapshot_backing_ppns(&self, out: &mut [Option<usize>]) -> usize {
        let _guard = self.lock();
        let mut written = 0usize;
        for list in [SlabList::Partial, SlabList::Full, SlabList::Empty] {
            let mut current = unsafe { *self.head_for(list) };
            while !current.is_null() {
                if let Some(entry) = out.get_mut(written) {
                    *entry = Some(unsafe { (*current).backing_ppn().0 });
                }
                written += 1;
