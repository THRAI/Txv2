...
use tx_substrate::page_allocator::{
    AllocError, AllocatorBackendKind, AllocatorDiagnostics, BitmapPageAllocator, FrameMeta,
    FrameReservation, FrameRunReservation, OwnedFrame, PageAllocator, RetainedFrame,
    RetainedFrameAllocator, ZeroPolicy,
};
...
struct StubPageAllocator<const RETAINED: bool> {
    _not_sync: Cell<()>,
}

impl<const RETAINED: bool> StubPageAllocator<RETAINED> {
    fn new() -> Self {
        Self {
            _not_sync: Cell::new(()),
        }
...
impl<const RETAINED: bool> PageAllocator for StubPageAllocator<RETAINED> {
    fn reserve_frame(&self, _policy: ZeroPolicy) -> Result<FrameReservation<'_, Self>, AllocError> {
    ...
    fn commit_reserved(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn commit_reserved_run(&self, _base: Ppn, _count: usize) {
        unimplemented!()
    }

    fn rollback_reserved(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn rollback_reserved_run(&self, _base: Ppn, _count: usize) {
        unimplemented!()
    }

    fn acquire_map_pin(&self, _ppn: Ppn) -> Result<(), AllocError> {
        unimplemented!()
    }

    fn release_map_pin(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn acquire_cache_pin(&self, _ppn: Ppn) -> Result<(), AllocError> {
        unimplemented!()
    }

    fn release_cache_pin(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn acquire_dma_pin(&self, _ppn: Ppn) -> Result<(), AllocError> {
        unimplemented!()
    }

    fn release_dma_pin(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn acquire_gift_pin(&self, _ppn: Ppn) -> Result<(), AllocError> {
        unimplemented!()
    }

    fn release_gift_pin(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn release_owned(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn adopt_page_table_frame(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn adopt_permanent_frame(&self, _ppn: Ppn) {
        unimplemented!()
    }

    fn release_page_table_frame(&self, _ppn: Ppn) {
        unimplemented!()
    }
}

impl RetainedFrameAllocator for StubPageAllocator<true> {
    fn acquire_retained_frame(&self, _ppn: Ppn) -> Result<(), AllocError> {
        Ok(())
    }

    fn release_retained_frame(&self, _ppn: Ppn) {}
}

#[test]
fn page_allocator_base_trait_remains_compatible_without_retention() {
    fn reserve_with_base_trait<A: PageAllocator>(
        allocator: &A,
    ) -> Result<FrameReservation<'_, A>, AllocError> {
        allocator.reserve_frame(ZeroPolicy::UninitFullOverwrite)
    }

    let allocator = StubPageAllocator::<false>::new();
    assert_eq!(allocator.free_count(), 0);
    assert_eq!(allocator.total_count(), 0);
    assert!(matches!(
        reserve_with_base_trait(&allocator),
        Err(AllocError::Exhausted)
    ));
}
...
#[test]
fn retained_frame_acquire_and_drop_changes_only_refcount() {
    let metas = [FrameMeta::new()];
    let bitmap = [AtomicU64::new(0)];
    let allocator = BitmapPageAllocator::new_for_test(&metas, &bitmap, 1);
    allocator.mark_free_for_test(Ppn(0));
    ...
    let retained = frame
        .try_retain()
        .expect("retained evidence should keep a live frame owned");

    assert_eq!(retained.ppn(), Ppn(0));
    assert_eq!(metas[0].refcount_for_test(), 2);
    assert_eq!(metas[0].map_count_for_test(), 1);
    assert_eq!(metas[0].cache_ref_for_test(), 1);
    assert_eq!(metas[0].pin_count_for_test(), 1);
...
#[test]
fn retained_frame_rejects_dead_frame() {
    let metas = [FrameMeta::new()];
    let bitmap = [AtomicU64::new(0)];
    let allocator = BitmapPageAllocator::new_for_test(&metas, &bitmap, 1);
    allocator.mark_free_for_test(Ppn(0));
    ...
    let err = allocator
        .acquire_retained_frame(Ppn(0))
        .expect_err("dead frame cannot gain retained-owner evidence");
...
#[test]
fn retained_frame_then_map_pin_drop_order_preserves_and_frees_frame() {
    let metas = [FrameMeta::new()];
    let bitmap = [AtomicU64::new(0)];
    let allocator = BitmapPageAllocator::new_for_test(&metas, &bitmap, 1);
    allocator.mark_free_for_test(Ppn(0));
    ...
    let frame = allocator
        ...
        .expect("reservation")
        .commit();
    let retained = frame.try_retain().expect("retain live frame");
    let map_pin = frame.try_map_pin().expect("map pin should fit");
...
#[test]
fn map_pin_then_retained_frame_drop_order_preserves_and_frees_frame() {
    let metas = [FrameMeta::new()];
    let bitmap = [AtomicU64::new(0)];
    let allocator = BitmapPageAllocator::new_for_test(&metas, &bitmap, 1);
    allocator.mark_free_for_test(Ppn(0));
    ...
    assert_eq!(metas[0].refcount_for_test(), 1);
    assert_eq!(metas[0].map_count_for_test(), 0);
    assert_eq!(allocator.free_count(), 0);
    assert!(!allocator.is_free_for_test(Ppn(0)));
...
#[test]
fn retained_frame_is_send_and_sync_when_allocator_is_sync() {
    assert_send_sync::<RetainedFrame<'static, BitmapPageAllocator<'static>>>();
}

#[test]
fn retained_frame_apis_do_not_require_sync_allocator() {
    fn assert_well_formed<T>() {}
    fn try_retain<'allocator, A: PageAllocator + RetainedFrameAllocator>(
        frame: &OwnedFrame<'allocator, A>,
    ) -> Result<RetainedFrame<'allocator, A>, AllocError> {
        frame.try_retain()
    }

    assert_well_formed::<RetainedFrame<'static, StubPageAllocator<true>>>();
    let _ = try_retain::<StubPageAllocator<true>>;
}
