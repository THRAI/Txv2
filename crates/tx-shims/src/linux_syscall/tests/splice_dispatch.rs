...
use tx_subsystems::vm::{MapPlacement, Prot, UserRange, UserVirtAddr, VmBacking, VmEntryFlags};
...
fn pipe_pair(ctx: &SyscallCtx<'_>) -> (u32, u32) {
    pipe_pair_with_flags(ctx, 0)
}

fn pipe_pair_with_flags(ctx: &SyscallCtx<'_>, flags: u32) -> (u32, u32) {
    let mut pipefd = [u32::MAX; 2];
    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(
            NR_PIPE2,
            [pipefd.as_mut_ptr() as u64, flags as u64, 0, 0, 0, 0],
        ),
        ctx,
    ));
...
fn fill_pipe_pages(ctx: &SyscallCtx<'_>, fd: u32, page_count: usize, byte: u8) {
    let page = vec![byte; tx_subsystems::vm::USER_PAGE_SIZE];
    for _ in 0..page_count {
        write_all(ctx, fd, &page);
    }
}

fn read_exact(ctx: &SyscallCtx<'_>, fd: u32, len: usize) -> Vec<u8> {
    read_some(ctx, fd, len, len)
}

fn read_some(ctx: &SyscallCtx<'_>, fd: u32, len: usize, expected: usize) -> Vec<u8> {
    let mut out = Vec::new();
    out.resize(len, 0);
    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(
    ...
    assert_eq!(result, SyscallResult::Return(expected as i64));
...
fn map_private_user_page(ctx: &SyscallCtx<'_>, addr: usize, bytes: &[u8]) -> u64 {
    let range = UserRange::new_aligned(UserVirtAddr(addr), tx_subsystems::vm::USER_PAGE_SIZE)
        .expect("aligned user page");
    ctx.aspace
        ...
        .expect("map private user page");
    let guard = guard();
    match ctx
        .aspace
        .copy_to_user(tx_hal::UserPtr::<u8>::new(addr), bytes, &guard)
    {
        StepOutcome::Done(n) => assert_eq!(n, bytes.len()),
        other => panic!("copy_to_user test payload: {other:?}"),
    }
    addr as u64
}
...
#[test]
fn dispatch_vmsplice_gift_full_page_then_splice_to_pagebacked_installs_page() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap.clone(), thread);
    let dst = pagebacked_open_file(9_507, 1, 0);
    proc_cap.set_fd(10, Some(dst.clone()));
    let (read_fd, write_fd) = pipe_pair(&ctx);
    let payload: Vec<u8> = (0..tx_subsystems::vm::USER_PAGE_SIZE)
        .map(|idx| (idx & 0xff) as u8)
        .collect();
    let user_addr = map_private_user_page(&ctx, 0x6200_0000, &payload);
    let iov = [TestIovec {
        base: user_addr,
    ...
    dst.set_offset(0);
    read_pagebacked(&dst, &mut observed);
    assert_eq!(observed, payload);
}

#[test]
fn dispatch_vmsplice_gift_unaligned_iovec_falls_back_to_copy() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap, thread);
    let (read_fd, write_fd) = pipe_pair(&ctx);
    let payload = b"unaligned-copy";
    let user_addr = map_private_user_page(&ctx, 0x6201_0000, payload);
    let iov = [TestIovec {
        base: user_addr + 1,
        len: (payload.len() - 1) as u64,
    }];
    ...
    assert_eq!(result, SyscallResult::Return((payload.len() - 1) as i64));
    assert_eq!(read_exact(&ctx, read_fd, payload.len() - 1), &payload[1..]);
}

#[test]
fn dispatch_vmsplice_gift_full_pipe_returns_eagain_without_detaching_source() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap, thread);
    let (read_fd, write_fd) = pipe_pair(&ctx);
    fill_pipe_pages(&ctx, write_fd, 16, b'x');

    let payload: Vec<u8> = (0..tx_subsystems::vm::USER_PAGE_SIZE)
        .map(|idx| 255 - (idx & 0xff) as u8)
        .collect();
    let user_addr = map_private_user_page(&ctx, 0x6202_0000, &payload);
    let source_page = UserVirtAddr(user_addr as usize).containing_page();
    assert!(
        ctx.aspace.pmap().lookup(source_page).is_some(),
        "source page must be materialized before gifting attempt"
    );
    let iov = [TestIovec {
        base: user_addr,
        len: payload.len() as u64,
    ...
    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(
            ...
            [
                ...
                iov.as_ptr() as u64,
                1,
                (SPLICE_F_GIFT | SPLICE_F_NONBLOCK) as u64,
                0,
                0,
            ],
        ),
    ...
    assert!(
        ctx.aspace.pmap().lookup(source_page).is_some(),
        "failed gift publication must not detach the source mapping"
    );
    assert_eq!(read_exact(&ctx, read_fd, 4), b"xxxx");
}
...
#[test]
fn dispatch_splice_to_packet_pipe_marks_output_packets() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap, thread);
    let (in_read, in_write) = pipe_pair(&ctx);
    let (out_read, out_write) = pipe_pair_with_flags(&ctx, O_DIRECT);
    write_all(&ctx, in_write, b"abcdef");
    write_all(&ctx, in_write, b"XY");

    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(NR_SPLICE, [in_read as u64, 0, out_write as u64, 0, 6, 0]),
        &ctx,
    ));
    assert_eq!(result, SyscallResult::Return(6));
    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(NR_SPLICE, [in_read as u64, 0, out_write as u64, 0, 2, 0]),
        &ctx,
    ));
    assert_eq!(result, SyscallResult::Return(2));

    assert_eq!(read_exact(&ctx, out_read, 3), b"abc");
    assert_eq!(read_some(&ctx, out_read, 4, 2), b"XY");
}
...
#[test]
fn dispatch_tee_to_packet_pipe_marks_output_packet() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap, thread);
    let (in_read, in_write) = pipe_pair(&ctx);
    ...
    let result = block_on(dispatch::<ShimsTestPmap>(
        SyscallRequest::new(NR_TEE, [in_read as u64, out_write as u64, 6, 0, 0, 0]),
        &ctx,
    ));
    assert_eq!(result, SyscallResult::Return(6));
    assert_eq!(read_exact(&ctx, out_read, 3), b"abc");
    assert_eq!(read_exact(&ctx, in_read, 6), b"abcdef");
}
...
#[test]
fn dispatch_splice_full_page_lease_survives_pipe_to_pipe_hop() {
    let _setup = splice_setup();
    let (proc_cap, thread) = fresh_proc_thread();
    let ctx = make_ctx(proc_cap.clone(), thread);
    let src = pagebacked_open_file(9_505, 1, tx_subsystems::vm::USER_PAGE_SIZE as u64);
    let dst = pagebacked_open_file(9_506, 1, 0);
    proc_cap.set_fd(9, Some(src.clone()));
    proc_cap.set_fd(10, Some(dst.clone()));
    let payload = alloc::vec![b'r'; tx_subsystems::vm::USER_PAGE_SIZE];
    write_pagebacked(&src, &payload);
    src.set_offset(0);
    let (pipe1_read, pipe1_write) = pipe_pair(&ctx);
    let (pipe2_read, pipe2_write) = pipe_pair(&ctx);
    ...
    assert_eq!(
        src_page, dst_page,
        "pipe-to-pipe splice should preserve the full-page lease descriptor"
    );
}
