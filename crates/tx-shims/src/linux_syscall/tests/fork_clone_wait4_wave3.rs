...
use crate::linux_syscall::{CLOCK_MONOTONIC, NR_TIMER_CREATE, NR_TIMER_SETTIME};
...
#[test]
fn dispatch_wait4_blocking_wakes_for_process_timer_signal_deadline() {
    let _setup = setup();
    install_noop_submit_seam();
    let proc_cap = bootstrap();
    let thread = first_thread(&proc_cap);
    ...
    let _ = block_on(dispatch::<ShimsTestPmap>(clone_req, &ctx));
    assert_eq!(proc_cap.children().len(), 1);
    assert!(!proc_cap.children()[0].is_zombie());
    ...
    assert!(
        result.is_ready(),
        "process timer deadline should wake blocked wait4"
    );

    let payload = thread.payload_cap().expect("live thread");
    assert!(
        payload.pending().is_pending(sigalrm),
        "wait4 process-timer wake should publish SIGALRM"
    );
    assert_eq!(
        proc_cap.children().len(),
        1,
        "interrupted wait4 must not reap the still-running child"
    );
}
