...
pub use numbers::{
    ...
    FUTEX_WAIT_BITSET, FUTEX_WAKE, FUTEX_WAKE_BITSET, FUTEX_WAKE_OP, F_ADD_SEALS, F_DUPFD,
    F_DUPFD_CLOEXEC, F_GETFD, F_GETFL, F_GETPIPE_SZ, F_GET_SEALS, F_OK, F_SEAL_FUTURE_WRITE,
    F_SEAL_GROW, F_SEAL_SEAL, F_SEAL_SHRINK, F_SEAL_WRITE, F_SETFD, F_SETFL, F_SETPIPE_SZ,
    GRND_INSECURE, GRND_NONBLOCK, GRND_RANDOM, ITIMER_PROF, ITIMER_REAL, ITIMER_VIRTUAL,
    ...
    MAP_STACK, MAP_SYNC, MCL_CURRENT, MCL_FUTURE, MCL_ONFAULT, MFD_ALLOW_SEALING, MFD_CLOEXEC,
    MFD_EXEC, MFD_HUGETLB, MFD_NOEXEC_SEAL, MLOCK_ONFAULT, MPOL_BIND, MPOL_DEFAULT, MPOL_F_ADDR,
    MPOL_F_MEMS_ALLOWED, MPOL_F_NODE, MPOL_F_RELATIVE_NODES, MPOL_F_STATIC_NODES, MPOL_INTERLEAVE,
    MPOL_LOCAL, MPOL_MF_MOVE, MPOL_MF_MOVE_ALL, MPOL_MF_STRICT, MPOL_PREFERRED,
    MPOL_PREFERRED_MANY, MREMAP_DONTUNMAP, MREMAP_FIXED, MREMAP_MAYMOVE, NR_ACCEPT, NR_ACCEPT4,
    NR_ADJTIMEX, NR_BIND, NR_BRK, NR_CHDIR, NR_CLOCK_ADJTIME, NR_CLOCK_GETRES, NR_CLOCK_GETTIME,
    NR_CLOCK_NANOSLEEP, NR_CLOCK_SETTIME, NR_CLONE, NR_CLOSE, NR_CLOSE_RANGE, NR_CONNECT,
    NR_COPY_FILE_RANGE, NR_DUP, NR_DUP3, NR_EPOLL_CREATE1, NR_EPOLL_CTL, NR_EPOLL_PWAIT,
    NR_EPOLL_PWAIT2, NR_EVENTFD2, NR_EXECVE, NR_EXECVEAT, NR_EXIT, NR_EXIT_GROUP, NR_FACCESSAT,
    NR_FACCESSAT2, NR_FADVISE64_64, NR_FALLOCATE, NR_FANOTIFY_INIT, NR_FANOTIFY_MARK, NR_FCHDIR,
    NR_FCHMOD, NR_FCHMODAT, NR_FCHMODAT2, NR_FCHOWN, NR_FCHOWNAT, NR_FCNTL, NR_FDATASYNC, NR_FLOCK,
    NR_FSTAT, NR_FSTATFS, NR_FSYNC, NR_FTRUNCATE, NR_FUTEX, NR_GETCPU, NR_GETCWD, NR_GETDENTS64,
    NR_GETEGID, NR_GETEUID, NR_GETGID, NR_GETGROUPS, NR_GETITIMER, NR_GETPEERNAME, NR_GETPGID,
    NR_GETPID, NR_GETPPID, NR_GETPRIORITY, NR_GETRANDOM, NR_GETRESGID, NR_GETRESUID, NR_GETRLIMIT,
    NR_GETRUSAGE, NR_GETSID, NR_GETSOCKNAME, NR_GETSOCKOPT, NR_GETTID, NR_GETTIMEOFDAY, NR_GETUID,
    NR_GET_MEMPOLICY, NR_GET_ROBUST_LIST, NR_INOTIFY_ADD_WATCH, NR_INOTIFY_INIT1,
    NR_INOTIFY_RM_WATCH, NR_IOCTL, NR_IOPRIO_GET, NR_IOPRIO_SET, NR_IO_DESTROY, NR_IO_GETEVENTS,
    NR_IO_PGETEVENTS, NR_IO_SETUP, NR_IO_SUBMIT, NR_IO_URING_ENTER, NR_IO_URING_SETUP, NR_KILL,
    NR_LINKAT, NR_LISTEN, NR_LSEEK, NR_MADVISE, NR_MBIND, NR_MEMFD_CREATE, NR_MIGRATE_PAGES,
    NR_MINCORE, NR_MKDIRAT, NR_MKNODAT, NR_MLOCK, NR_MLOCK2, NR_MLOCKALL, NR_MMAP, NR_MOUNT,
    NR_MOVE_PAGES, NR_MPROTECT, NR_MQ_GETSETATTR, NR_MQ_NOTIFY, NR_MQ_OPEN, NR_MQ_TIMEDRECEIVE,
    NR_MQ_TIMEDSEND, NR_MQ_UNLINK, NR_MREMAP, NR_MSGCTL, NR_MSGGET, NR_MSGRCV, NR_MSGSND, NR_MSYNC,
    NR_MUNLOCK, NR_MUNLOCKALL, NR_MUNMAP, NR_NAME_TO_HANDLE_AT, NR_NANOSLEEP, NR_NEWFSTATAT,
    NR_OPENAT, NR_OPENAT2, NR_OPEN_BY_HANDLE_AT, NR_PERSONALITY, NR_PIDFD_OPEN,
    NR_PIDFD_SEND_SIGNAL, NR_PIPE2, NR_PPOLL, NR_PREAD64, NR_PREADV, NR_PREADV2, NR_PRLIMIT64,
    NR_PROCESS_MADVISE, NR_PROCESS_VM_READV, NR_PROCESS_VM_WRITEV, NR_PWRITE64, NR_PWRITEV,
    NR_PWRITEV2, NR_READ, NR_READAHEAD, NR_READLINKAT, NR_READV, NR_RECVFROM, NR_REMAP_FILE_PAGES,
    NR_RENAMEAT2, NR_RESTART_SYSCALL, NR_RT_SIGACTION, NR_RT_SIGPENDING, NR_RT_SIGPROCMASK,
    NR_RT_SIGQUEUEINFO, NR_RT_SIGRETURN, NR_RT_SIGSUSPEND, NR_RT_SIGTIMEDWAIT,
    NR_SCHED_GETAFFINITY, NR_SCHED_GETPARAM, NR_SCHED_GETSCHEDULER, NR_SCHED_GET_PRIORITY_MAX,
    NR_SCHED_GET_PRIORITY_MIN, NR_SCHED_RR_GET_INTERVAL, NR_SCHED_SETAFFINITY, NR_SCHED_SETPARAM,
    NR_SCHED_SETSCHEDULER, NR_SCHED_YIELD, NR_SEMCTL, NR_SEMGET, NR_SEMOP, NR_SEMTIMEDOP,
    NR_SENDTO, NR_SETGID, NR_SETITIMER, NR_SETPGID, NR_SETPRIORITY, NR_SETREGID, NR_SETRESGID,
    NR_SETRESUID, NR_SETREUID, NR_SETRLIMIT, NR_SETSID, NR_SETSOCKOPT, NR_SETTIMEOFDAY, NR_SETUID,
    NR_SET_MEMPOLICY, NR_SET_ROBUST_LIST, NR_SET_TID_ADDRESS, NR_SHMAT, NR_SHMCTL, NR_SHMDT,
    NR_SHMGET, NR_SHUTDOWN, NR_SIGALTSTACK, NR_SIGNALFD4, NR_SOCKET, NR_SOCKETPAIR, NR_SPLICE,
    NR_STATFS, NR_STATX, NR_SYMLINKAT, NR_SYNC, NR_SYNCFS, NR_SYNC_FILE_RANGE, NR_SYSLOG, NR_TEE,
    NR_TGKILL, NR_TIMERFD_CREATE, NR_TIMERFD_GETTIME, NR_TIMERFD_SETTIME, NR_TIMER_CREATE,
    NR_TIMER_DELETE, NR_TIMER_GETOVERRUN, NR_TIMER_GETTIME, NR_TIMER_SETTIME, NR_TIMES, NR_TKILL,
    NR_TRUNCATE, NR_UMASK, NR_UMOUNT2, NR_UNAME, NR_UNLINKAT, NR_USERFAULTFD, NR_UTIMENSAT,
    NR_VMSPLICE, NR_WAIT4, NR_WRITE, NR_WRITEV, O_ACCMODE, O_APPEND, O_CLOEXEC, O_CREAT, O_DIRECT,
    O_EXCL, O_NONBLOCK, O_PATH, O_RDONLY, O_RDWR, O_TRUNC, O_WRONLY, PROT_EXEC, PROT_GROWSDOWN,
...
async fn dispatch_inner<'a, P: PmapIf + EntropyIf + TimeIf + AuxvIf + SmpIf>(
    ...
    poll_expired_process_timers::<P>(ctx);
    ...
    match req.nr {
        ...
        nr if nr == NR_REMAP_FILE_PAGES => return sys_remap_file_pages(req.args, ctx).await,
        ...
        nr if nr == NR_MEMFD_CREATE => return sys_memfd_create(req.args, ctx),
    ...
    match req.nr {
        ...
        nr if nr == NR_MQ_TIMEDSEND => sys_mq_timedsend::<P>(req.args, ctx).await,
        nr if nr == NR_MQ_TIMEDRECEIVE => sys_mq_timedreceive::<P>(req.args, ctx).await,
        ...
        nr if nr == NR_FSETXATTR => sys_fsetxattr(req.args, ctx),
        nr if nr == NR_GETXATTR => sys_getxattr_path(req.args, ctx, false),
        nr if nr == NR_LGETXATTR => sys_getxattr_path(req.args, ctx, true),
        nr if nr == NR_FGETXATTR => sys_fgetxattr(req.args, ctx),
        nr if nr == NR_LISTXATTR => sys_listxattr_path(req.args, ctx, false),
        nr if nr == NR_LLISTXATTR => sys_listxattr_path(req.args, ctx, true),
        nr if nr == NR_FLISTXATTR => sys_flistxattr(req.args, ctx),
        nr if nr == NR_REMOVEXATTR => sys_removexattr_path(req.args, ctx, false),
        nr if nr == NR_LREMOVEXATTR => sys_removexattr_path(req.args, ctx, true),
        nr if nr == NR_FREMOVEXATTR => sys_fremovexattr(req.args, ctx),
        nr if nr == NR_SETXATTRAT => sys_setxattrat(req.args, ctx),
        nr if nr == NR_GETXATTRAT => sys_getxattrat(req.args, ctx),
        nr if nr == NR_LISTXATTRAT => sys_listxattrat(req.args, ctx),
        nr if nr == NR_REMOVEXATTRAT => sys_removexattrat(req.args, ctx),
        ...
        nr if nr == NR_OPENAT2 => sys_openat2::<P>(req.args, ctx).await,
        ...
        nr if nr == NR_ADJTIMEX => sys_adjtimex::<P>(req.args, ctx),
        nr if nr == NR_CLOCK_ADJTIME => sys_clock_adjtime::<P>(req.args, ctx),
        ...
        // deferred temporary-mask policy used by ppoll/epoll_pwait.
        nr if nr == NR_IO_PGETEVENTS => sys_io_pgetevents(req.args, ctx).await,
