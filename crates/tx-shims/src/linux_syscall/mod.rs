...
pub use numbers::{
    ...
    DT_FIFO, DT_LNK, DT_REG, DT_SOCK, DT_UNKNOWN, FD_CLOEXEC, FUTEX_32, FUTEX_CLOCK_REALTIME,
    ...
    NR_FSTAT, NR_FSTATFS, NR_FSYNC, NR_FTRUNCATE, NR_FUTEX, NR_FUTEX2_REQUEUE, NR_FUTEX2_WAIT,
    NR_FUTEX2_WAKE, NR_FUTEX_WAITV, NR_GETCPU, NR_GETCWD, NR_GETDENTS64, NR_GETEGID, NR_GETEUID,
    NR_GETGID, NR_GETGROUPS, NR_GETPEERNAME, NR_GETPGID, NR_GETPID, NR_GETPPID, NR_GETPRIORITY,
    NR_GETRANDOM, NR_GETRESGID, NR_GETRESUID, NR_GETRLIMIT, NR_GETRUSAGE, NR_GETSID,
    NR_GETSOCKNAME, NR_GETSOCKOPT, NR_GETTID, NR_GETTIMEOFDAY, NR_GETUID, NR_GET_ROBUST_LIST,
    ...
    NR_IO_URING_SETUP, NR_KILL, NR_LINKAT, NR_LISTEN, NR_LSEEK, NR_MADVISE, NR_MKDIRAT, NR_MKNODAT,
    NR_MLOCK, NR_MMAP, NR_MOUNT, NR_MPROTECT, NR_MQ_GETSETATTR, NR_MQ_NOTIFY, NR_MQ_OPEN,
    NR_MQ_TIMEDRECEIVE, NR_MQ_TIMEDSEND, NR_MQ_UNLINK, NR_MREMAP, NR_MSGCTL, NR_MSGGET, NR_MSGRCV,
    NR_MSGSND, NR_MSYNC, NR_MUNLOCK, NR_MUNMAP, NR_NANOSLEEP, NR_NEWFSTATAT, NR_OPENAT,
...
async fn dispatch_inner<'a, P: PmapIf + EntropyIf + TimeIf + AuxvIf + SmpIf>(
    ...
    match req.nr {
        ...
        nr if nr == NR_FUTEX_WAITV => sys_futex_waitv::<P>(req.args, ctx).await,
        nr if nr == NR_FUTEX2_WAKE => sys_futex2_wake(req.args, ctx),
        nr if nr == NR_FUTEX2_WAIT => sys_futex2_wait::<P>(req.args, ctx).await,
        nr if nr == NR_FUTEX2_REQUEUE => sys_futex2_requeue(req.args, ctx),
