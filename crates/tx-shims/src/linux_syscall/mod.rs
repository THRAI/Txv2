...
async fn dispatch_inner<'a, P: PmapIf + EntropyIf + TimeIf + AuxvIf + SmpIf>(
    ...
    match req.nr {
        ...
        nr if nr == NR_ACCEPT => sys_accept(req.args, ctx).await,
        nr if nr == NR_ACCEPT4 => sys_accept4(req.args, ctx).await,
        ...
        nr if nr == NR_RECVFROM => sys_recvfrom(req.args, ctx).await,
