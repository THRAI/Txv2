...
fn build_tier1_cross_group_mkdir_image() -> MemImage {
    let mut image = build_tier1_mount_image();
    image.blocks.resize(192, [0; BLOCK_SIZE]);

    let mut superblock = Superblock::parse(&image.block_mut(0)[1024..2048]).expect("superblock");
    superblock.blocks_count = 192;
    superblock.inodes_count = 192;
    superblock.free_blocks_count = 57;
    superblock.free_inodes_count = 64;
    superblock
        .encode(&mut image.block_mut(0)[1024..2048])
        .expect("encode cross-group superblock");

    // Force the same allocation shape as the official finals image: the
    // parent is in group 0, the first free data block is in group 1, and the
    // first free inode is in group 2. This also makes the new inode table a
    // distinct metadata home from the parent's inode table.
    ...
    {
        descriptor
            .encode(&mut image.block_mut(1)[offset * 32..(offset + 1) * 32])
            .expect("encode cross-group descriptor");
    }

    image.block_mut(2).fill(0xff);
    image.block_mut(3).fill(0xff);
    {
        let mut bitmap = BitmapMut::new(image.block_mut(65));
        for bit in 0..7 {
            bitmap.set(bit).expect("reserve group-1 metadata block");
        }
    }
    image.block_mut(66).fill(0xff);
    image.block_mut(129).fill(0xff);
    image.block_mut(130).fill(0);
    image
}
...
fn mutation_runtime_for_test_with_ring(
    ...
    mutation_runtime_for_test_with_ring_pool(sequence, metadata_slots, with_revoke, 16)
}

fn mutation_runtime_for_test_with_ring_pool(
    sequence: u32,
    metadata_slots: u64,
    with_revoke: bool,
    page_capacity: u64,
    ...
    Arc::new(
        JournalMutationRuntime::from_geometry_with_sequence(
            ...
            JournalPagePool::new(page_capacity).expect("journal pool"),
            DeviceKey::new(7),
            8,
...
fn mounted_counting_write_growth_fs_with_ring(
    sequence: u32,
) -> (
    crate::mount::MountedExt4<CountingImage>,
    Arc<JournalMutationRuntime>,
    ...
    let writes = Arc::new(AtomicUsize::new(0));
    let runtime = mutation_runtime_for_test_with_ring(sequence, 4, false);
    let mounted = mount_ext4_read_write_with_mutation_journal_io_manager_planner(
        CountingImage {
            image: build_tier1_mount_image(),
            writes: Arc::clone(&writes),
    ...
    .expect("mount Tier 1 mutation ext4 image with ring");
    (mounted, runtime, writes)
}
...
fn mounted_counting_cross_group_mkdir_fs_with_ring(
    sequence: u32,
) -> (
    crate::mount::MountedExt4<CountingImage>,
    Arc<JournalMutationRuntime>,
    ...
    let writes = Arc::new(AtomicUsize::new(0));
    let runtime = mutation_runtime_for_test_with_ring_pool(sequence, 8, false, 32);
    let mounted = mount_ext4_read_write_with_mutation_journal_io_manager_planner(
        CountingImage {
            image: build_tier1_cross_group_mkdir_image(),
            writes: Arc::clone(&writes),
        },
        Ext4BlockGeometry::new(DeviceKey::new(7), 8),
        Arc::clone(&runtime),
    )
    .expect("mount Tier 1 cross-group mkdir image with ring");
    (mounted, runtime, writes)
}
...
#[test]
fn ext4_metadata_admission_yields_and_wakes_instead_of_spinning() {
    let _serial = EXT4_V3_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    init_substrate();
    let fs = open_fs();
    let permit = fs
    ...
    assert!(fs.try_lock_metadata_mutation_admission().is_none());
    ...
    assert_eq!(endpoint.pending_mask_snapshot(), 0);
...
fn ext4_buffered_extending_write_defers_allocation_until_writeback() {
    ...
    init_substrate();
    let guard = epoch::guard();
    let (mounted, runtime, writes) = mounted_counting_write_growth_fs_with_ring(24);
...
fn ext4_allows_multiple_extending_buffered_writes_before_writeback() {
    ...
    init_substrate();
    let guard = epoch::guard();
    let (mounted, runtime, writes) = mounted_counting_write_growth_fs_with_ring(25);
    let pc = page_container_for_mounted_file(&mounted, FsObjectId::new(12), BLOCK_SIZE as u64, 8);
    let of = open_file_for_page_container(&pc);
...
fn ext4_truncate_with_deferred_dirty_growth_settles_cleanly() {
    ...
    init_substrate();
    let guard = epoch::guard();
    let (mounted, runtime, writes) = mounted_counting_write_growth_fs_with_ring(26);
    let pc = page_container_for_mounted_file(&mounted, FsObjectId::new(12), BLOCK_SIZE as u64, 8);
    let of = open_file_for_page_container(&pc);
...
#[test]
fn ext4_inode_reuse_does_not_reuse_live_unlinked_page_container() {
    let _serial = EXT4_V3_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    init_substrate();
    let guard = epoch::guard();
    let cred = tx_subsystems::vfs::Credential::root();
    let (mounted, runtime, writes) = mounted_counting_create_fs_with_ring(42);
    let fs_ops = mounted.fs_ops();
    let (_root, _mount, payload) = ext4_root_dentry_for_mounted(&mounted, 8, 42);

    let (first_id, first_meta) =
        match fs_ops.create_inode(FsObjectId::new(2), b"first", 0o100640, &cred, &guard) {
            V3::Done(created) => created,
            other => panic!("create first file for page-container reuse test: {other:?}"),
        };
    let first_rnode = match fs_ops.materialise_rnode(first_id, first_meta, &payload, &guard) {
        V3::Done(rnode) => rnode,
        other => panic!("materialise first inode: {other:?}"),
    };
    let first_pc = match first_rnode.backing() {
        RNodeBacking::PageBacked { pc } => pc.clone(),
        other => panic!("first inode is not page-backed: {other:?}"),
    };
    first_pc.set_size_bytes(BLOCK_SIZE as u64);
    drop(first_rnode);
    ...
    let second_rnode = match fs_ops.materialise_rnode(second_id, second_meta, &payload, &guard) {
        V3::Done(rnode) => rnode,
        other => panic!("materialise replacement inode: {other:?}"),
    };
    let second_pc = match second_rnode.backing() {
        RNodeBacking::PageBacked { pc } => pc.clone(),
        other => panic!("replacement inode is not page-backed: {other:?}"),
    };
    assert_ne!(second_pc, first_pc);
    assert_eq!(second_pc.size_bytes(), 0);
    assert_eq!(first_pc.size_bytes(), BLOCK_SIZE as u64);
    assert_metadata_settled(&runtime, &writes);
}
...
#[test]
fn ext4_public_path_renames_new_directory_within_same_parent() {
    let _serial = EXT4_V3_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    init_substrate();
    let guard = epoch::guard();
    let cred = tx_subsystems::vfs::Credential::root();
    let (mounted, runtime, writes) = mounted_counting_mkdir_fs_with_ring(49);
    let fs_ops = mounted.fs_ops();

    let (dir_id, _) = match fs_ops.mkdir(FsObjectId::new(2), b"target-temp", 0o755, &cred, &guard) {
        V3::Done(created) => created,
        other => panic!("mkdir target-temp for same-parent rename: {other:?}"),
    };
    assert_metadata_settled(&runtime, &writes);
    ...
    assert_metadata_settled(&runtime, &writes);
    assert_eq!(
        fs_ops.lookup(FsObjectId::new(2), b"target", &guard),
        V3::<_, NoProgress>::done(dir_id)
    );
    assert!(matches!(
        fs_ops.lookup(FsObjectId::new(2), b"target-temp", &guard),
        V3::Err(_)
    ));
}
...
#[test]
fn ext4_mkdir_public_path_settles_cross_group_inode_and_data_with_ring_runtime() {
    let _serial = EXT4_V3_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    init_substrate();
    let guard = epoch::guard();
    let cred = tx_subsystems::vfs::Credential::root();
    let (mounted, runtime, writes) = mounted_counting_cross_group_mkdir_fs_with_ring(40);

    let (created, meta) =
        match mounted
            .fs_ops()
            .mkdir(FsObjectId::new(2), b"cross-group", 0o755, &cred, &guard)
        {
            V3::Done(created) => created,
...
#[test]
fn ext4_v3_filesystem_stats_reads_allocation_bitmaps() {
    let _serial = EXT4_V3_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    init_substrate();
    let mut image = build_image();
    mark_block_bitmap_used(&mut image, 32);
    ...
    assert_eq!(
        <Ext4FsInstance<MemImage> as FsPageBacking>::filesystem_stats(&*fs, &guard),
        V3::<_, NoProgress>::done(tx_subsystems::page_backed::FilesystemStats {
