...
fn pid_from_dir(id: FsObjectId) -> Option<Pid> {
    let r = id.as_u64();
    if r > PROCFS_PID_BASE && r < PROCFS_PID_BASE + 0x10000 { Some(Pid((r - PROCFS_PID_BASE) as u32)) } else { None }
}
...
impl FsOps for Procfs {
    ...
    fn load_inode_meta(&self, id: FsObjectId, _guard: &Guard<'_>) -> StepOutcome<InodeMeta, NoProgress> {
        match id {
            ...
            PROCFS_MOUNTS_ID => StepOutcome::done(InodeMeta::new(InodeKind::Regular, PROCFS_FILE_MODE)),
